...
namespace api_erp.Controllers.OSControllers
{
    ...
    {
        private readonly IKMRepository _repo;

        public KmsController(IKMRepository repo)
        {
            _repo = repo;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] KM entity, CancellationToken ct = default)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] KM entity, CancellationToken ct = default)
        {
