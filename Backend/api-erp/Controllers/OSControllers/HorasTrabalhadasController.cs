...
namespace api_erp.Controllers.OSControllers
{
    ...
    {
        private readonly IHoraTrabalhadaRepository _repo;

        public HorasTrabalhadasController(IHoraTrabalhadaRepository repo)
        {
            _repo = repo;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] HoraTrabalhada entity, CancellationToken ct = default)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] HoraTrabalhada entity, CancellationToken ct = default)
        {
